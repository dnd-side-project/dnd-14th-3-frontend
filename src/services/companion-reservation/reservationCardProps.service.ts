...
function createBaseProps(
  ...
  ReservationCardProps,
  "title" | "dateLabel" | "timeLabel" | "locationLabel" | "tags" | "className"
> {
...
// ─── 둘러보기 카드 ────────────────────────────────────────────
export function toBrowseCardProps(reservation: ReservationSummaryDto): ReservationCardProps {
  const base = createBaseProps(reservation);
  const config = RESERVATION_CARD_STATUS_CONFIG.recruiting;

  const requesterInfo = reservation.ownerProfileImageUrl
    ? {
