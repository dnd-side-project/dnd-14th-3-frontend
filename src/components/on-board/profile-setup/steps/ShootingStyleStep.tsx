...
export default function ShootingStyleStep() {
  const preferredStyles = useProfileSetupStore((s) => s.data.preferredStyles ?? []);
  const toggleShootingStyle = useProfileSetupStore((s) => s.toggleShootingStyle);
  const {
    canGoNext,
    goNext,
    goBack,
            ...
            {styles.map((style) => {
              ...
              return (
                <ChipButton
                  key={style.id}
                  selected={isSelected}
                  onClick={() => toggleShootingStyle(style.id)}
                >
                  {style.label}
                </ChipButton>
              );
